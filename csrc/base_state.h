...
template <bool abrupt> struct BaseState {
  ...
  // - the first bit (`move & 1`) is 1 if the move was successful, or 0 if the opponent had played there before 
  // - the next 4 bits contain the move, from 1 to 9 (ie. `((move >> 1) & 0b1111) - 1` contains the cell between 0 and 8)
  ...
  uint64_t get_infoset() const {
    ...
    // -> there have only been t_ moves played, so set all bits past the (5*t_)'s bit to 0 
